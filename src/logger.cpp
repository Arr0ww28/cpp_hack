/**
 * @file logger.cpp
 * @brief Implementation of EventLogger, LogEntry, and global logging functions.
 *
 * The EventLogger writes log entries to both a file and in-memory history.
 * It uses a ThreadSafeQueue for async decoupling — producers call logEvent()
 * from any thread, and the dedicated logger thread drains via processPendingEvents().
 *
 * C++ Concepts Demonstrated:
 *   - RAII (ofstream lifetime management)
 *   - Move Semantics (LogEntry transfer through queue)
 *   - STL Algorithms (std::copy_if for search)
 *   - Lambdas (predicate-based filtering)
 *   - Exception Handling (file open failures)
 *   - Mutex / lock_guard (history access)
 */
...
/**
 * @brief Generate a timestamp string in "YYYY-MM-DD HH:MM:SS" format.
 */
...
std::ostream& operator<<(std::ostream& os, const LogEntry& entry) {
    os << "[" << entry.timestamp << "] "
       ...
       << "[" << std::left << std::setw(28) << entry.source << "] "
       << entry.message;
    return os;
}
...
void EventLogger::log(LogLevel level, const std::string& source,
...
void EventLogger::processPendingEvents() {
    ...
    while (pendingQueue_.tryPop(entry)) {
        // Write to file (no ANSI codes — plain text)
        if (fileOpen_ && logFile_.is_open()) {
            logFile_ << entry << "\n" << std::flush;
        }
    ...
    std::function<bool(const LogEntry&)> predicate) const
{
    ...
    // STL algorithm with lambda predicate
    std::copy_if(eventHistory_.begin(), eventHistory_.end(),
...
std::vector<LogEntry> EventLogger::getRecentEvents(size_t count) const {
    ...
    size_t start = (eventHistory_.size() > count)
                   ? (eventHistory_.size() - count) : 0;

    for (size_t i = start; i < eventHistory_.size(); ++i) {
        results.push_back(eventHistory_[i]);
    }
    return results;
}
...
// =============================================================================
// Global Logger — Singleton Access
// =============================================================================
